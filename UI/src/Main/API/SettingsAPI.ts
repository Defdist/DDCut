...
class SettingsAPI {
    static Initialize() {
        ...
        ipcMain.on("Settings::GetFeedRate", function (event: Event) {
            console.log("Getting feedrate");
            event.returnValue = DDCut.GetFeedRate();
        });

        ipcMain.on("Settings::SetFeedRate", function (event: Event, feedRate: number) {    
            console.log("Setting feedrate");       
